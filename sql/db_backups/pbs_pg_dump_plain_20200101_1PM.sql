...
--
-- Name: vw_donor_stats_desc; Type: VIEW; Schema: bx; Owner: gastil
--

CREATE VIEW bx.vw_donor_stats_desc AS
 SELECT vw_donor_stats.donor,
    vw_donor_stats.n_items,
    vw_donor_stats.n_bxs,
    vw_donor_stats.n_taxa,
    vw_donor_stats.n_categories,
    vw_donor_stats.first_donation_date,
    vw_donor_stats.last_donation_date
   FROM bx.vw_donor_stats
  ORDER BY vw_donor_stats.n_items DESC, vw_donor_stats.n_bxs DESC, vw_donor_stats.n_taxa DESC, vw_donor_stats.n_categories DESC, vw_donor_stats.donor;


ALTER TABLE bx.vw_donor_stats_desc OWNER TO gastil;
...
--
-- Name: vw_most_oft_donated_genera_over_time; Type: VIEW; Schema: bx; Owner: gastil
--

CREATE VIEW bx.vw_most_oft_donated_genera_over_time AS
 SELECT split_part((i.taxon)::text, ' '::text, 1) AS genus,
    count(DISTINCT i.taxon) AS n_taxa,
    count(*) AS n_items,
    count(DISTINCT d.bx_id) AS n_bx,
    count(DISTINCT i.donor) AS n_donor,
    count(DISTINCT date_part('month'::text, d.date_open)) AS n_months,
    min(date_part('month'::text, d.date_open)) AS first_month,
    max(date_part('month'::text, d.date_open)) AS last_month,
    count(DISTINCT d.director_initials) AS n_drs,
    min(date_part('year'::text, d.date_open)) AS first_yr,
    max(date_part('year'::text, d.date_open)) AS last_yr,
    count(DISTINCT date_part('year'::text, d.date_open)) AS n_yrs,
        CASE min(i.category)
            WHEN 'BULBS'::bpchar THEN 'B'::text
            WHEN 'SEEDS'::bpchar THEN NULL::text
            ELSE 'error'::text
        END AS as_bulb,
        CASE max(i.category)
            WHEN 'SEEDS'::bpchar THEN 'S'::text
            WHEN 'BULBS'::bpchar THEN NULL::text
            ELSE 'error'::text
        END AS as_seed
   FROM (bx.bx_items i
     JOIN bx.bx_dates d ON (((i.bx_id)::text = (d.bx_id)::text)))
  GROUP BY (split_part((i.taxon)::text, ' '::text, 1))
  ORDER BY (count(DISTINCT i.taxon)) DESC, (count(*)) DESC, (split_part((i.taxon)::text, ' '::text, 1));


ALTER TABLE bx.vw_most_oft_donated_genera_over_time OWNER TO gastil;
...
--
-- Name: vw_n_dates_per_year; Type: VIEW; Schema: bx; Owner: gastil
--

CREATE VIEW bx.vw_n_dates_per_year AS
 SELECT "substring"((bx_dates.bx_id)::text, 1, 2) AS bs,
    date_part('year'::text, bx_dates.date_open) AS yr,
    count(*) AS n
   FROM bx.bx_dates
  GROUP BY ("substring"((bx_dates.bx_id)::text, 1, 2)), (date_part('year'::text, bx_dates.date_open))
  ORDER BY (date_part('year'::text, bx_dates.date_open)), ("substring"((bx_dates.bx_id)::text, 1, 2));


ALTER TABLE bx.vw_n_dates_per_year OWNER TO gastil;
